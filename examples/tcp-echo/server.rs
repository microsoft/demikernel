...
impl TcpEchoServer {
    pub fn new(mut libos: LibOS, local: SocketAddr) -> Result<Self> {
        let local_sockqd: QDesc = libos.socket(AF_INET, SOCK_STREAM, 0)?;

        if let Err(e) = libos.bind(local_sockqd, local) {
            println!("ERROR: {:?}", e);
            libos.close(local_sockqd)?;
            anyhow::bail!("{:?}", e);
        }

        if let Err(e) = libos.listen(local_sockqd, 1024) {
            println!("ERROR: {:?}", e);
            libos.close(local_sockqd)?;
            anyhow::bail!("{:?}", e);
        }
    ...
    pub fn run(&mut self, log_interval: Option<u64>) -> Result<()> {
        ...
        {
            let qt: QToken = self.libos.accept(self.local_sockqd)?;
    ...
    fn issue_accept(&mut self) -> Result<()> {
        let qt: QToken = self.libos.accept(self.local_sockqd)?;
        self.register_operation(self.local_sockqd, qt);
        Ok(())
    }
...
impl Drop for TcpEchoServer {
    fn drop(&mut self) {
        ...
        if let Err(e) = self.handle_close(self.local_sockqd) {
            println!("ERROR: {:?}", e);
        }
    }
}
