...
/// The client will send a packet to the server and wait for the server to echo it back. This is done in a closed loop
/// fashion, i.e., the client will not send a new packet until it receives the previous one.
pub struct TcpEchoClosedLoopClient {
    /// Underlying libOS.
    libos: LibOS,
    /// Buffer size.
    bufsize: usize,
...
impl TcpEchoClosedLoopClient {
    ...
    pub fn run_main_loop(
        ...
        connect_handler: fn(&mut TcpEchoClosedLoopClient, &demi_qresult_t) -> Result<()>,
    ) -> Result<()> {
    ...
    pub fn run_sequential(
        ...
        self.run_main_loop(
            ...
            nclients,
            nrequests,
            |_: &mut TcpEchoClosedLoopClient, qr: &demi_qresult_t| -> Result<()> {
...
impl Drop for TcpEchoClosedLoopClient {
    fn drop(&mut self) {
        // Close all connections.
        for (qd, _) in self.clients.drain().collect::<Vec<_>>() {
