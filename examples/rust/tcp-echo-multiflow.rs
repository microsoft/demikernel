...
fn worker_fn(args: &mut WorkerArg) -> ! {
    ...
    // Create the LibOS
    let mut libos: LibOS = LibOS::new(LibOSName::Catnip, port_id, worker_id, args.memory_manager.clone(), args.rxq_lock, args.q_ptr, args.shared_between_cores).unwrap();
...
pub fn main() -> Result<()> {
    ...
    if args.len() >= 5 {
        if args[1] == "--server" {
            ...
            // Creating the RXQ lock
            let rxq_lock: *mut DPDKSpinLock = Box::into_raw(Box::new(DPDKSpinLock::new()));

            // Creating the Q queue
            let q_ptr: *mut DPDKRing = Box::into_raw(Box::new(DPDKRing::new(RING_SIZE as u32)));
