...
class TcpEchoTest(BaseTest):
    ...
    def run_scenario(self, params):
        s = self.scaffolding
        is_debug: bool = s["is_debug"]
        test_alias = self.__make_test_alias(params)
        server_args = self.__make_server_args()
        client_args = self.__make_client_args(params, is_debug)
        self.has_test_passed = self.job_test_system_rust(
            test_alias, self.test_name, s["repository"], s["libos"], is_debug, s["server_name"],
    ...
    def __make_client_args(self, params, is_debug):
        if is_debug:
            ...
            return f"--log 5 --peer client --address {self.scaffolding['server_ip']}:12345 --nclients {params['nclients']} " \
                f"--nrequests {params['nrequests']} --bufsize {params['bufsize']} --run-mode {params['run_mode']}"
