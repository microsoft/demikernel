...
def main():
    ...
    __plot_performance(table_name=table_name, container_name=container_name,
                       connection_str=connection_str, key=key, no_plot=no_plot, libos=libos)
...
def __read_args() -> argparse.Namespace:
    ...
    parser.add_argument("--libos", required=False, help="Set the library OS to plot performance statistics for.")
...
def check_if_head_commit(commit_hash: str) -> bool:
    head_commit: str = get_head_commit()
    True if commit_hash == head_commit else False
...
def get_distance_of_commits(head_commit: str) -> int:
    base_commit = get_first_commit_on_branch(head_commit)
...
def __plot_performance(table_name: str, container_name: str, connection_str: str, key: str, no_plot: bool, libos: str) -> None:
    ...
    print(f"Processing commits form {root_commit} to {head_commit}...")
