...
extern "C"
{
    ...
    typedef void (*dmtr_onfail_t)(int error_arg, const char *expr_arg, const char *funcn_arg, const char *filen_arg,
                                  int lineno_arg);
...
#define DMTR_TRUE(Error, Condition) DMTR_TRUE2(Error, Condition, DMTR_TRUE_errorCache)
...
#define DMTR_OK(Error) DMTR_OK2((Error), DMTR_UNIQID(DMTR_OK_errorCache))
...
#define DMTR_PANIC(Why) dmtr_panic((Why), __FILE__, __LINE__)
    ...
    void dmtr_fail(int error_arg, const char *expr_arg, const char *funcn_arg, const char *filen_arg, int lineno_arg);
