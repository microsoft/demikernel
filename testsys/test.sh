...
if [ $? -eq 0 ]
then
    for test in "udp_push_pop" "udp_ping_pong" "tcp_ping_pong_single" "tcp_push_pop";
    do
        echo "Running $test"
        python3 $THIS_DIR/jobs/launch.py -w $WORKSPACE -p $WORKSPACE -s $SERVER -c $CLIENT -l $LIBOS -t $test
        test $? -eq 0 || status=1
    done
fi
...
test $status && echo "All testes have passed"
