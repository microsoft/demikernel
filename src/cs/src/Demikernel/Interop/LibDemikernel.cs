...
#pragma warning disable CS1591 // missing comments; these are **raw** APIs - please see primary source
    ...
    public static extern ApiResult socket(int* qd, AddressFamily domain, SocketType type, ProtocolType protocol);

    [DllImport("libdemikernel", EntryPoint = "demi_listen")]
    public static extern ApiResult listen(int qd, int backlog);

    [DllImport("libdemikernel", EntryPoint = "demi_bind")]
    public static extern ApiResult bind(int qd, byte* saddr, int size);

    [DllImport("libdemikernel", EntryPoint = "demi_accept")]
    public static extern ApiResult accept(long* qt, int sockqd);

    [DllImport("libdemikernel", EntryPoint = "demi_connect")]
    public static extern ApiResult connect(long* qt, int qd, byte* saddr, int size);

    [DllImport("libdemikernel", EntryPoint = "demi_close")]
    public static extern ApiResult close(int qd);

    [DllImport("libdemikernel", EntryPoint = "demi_push")]
    public static extern ApiResult push(long* qt, int qd, ScatterGatherArray* sga);

    [DllImport("libdemikernel", EntryPoint = "demi_pushto")]
    public static extern ApiResult pushto(long* qt, int qd, ScatterGatherArray* sga, byte* saddr, int size);

    [DllImport("libdemikernel", EntryPoint = "demi_pop")]
    public static extern ApiResult pop(long* qt, int qd);
