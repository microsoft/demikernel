...
//==============================================================================
#[macro_export]
macro_rules! __ak_time_log {
    ($($arg:tt)*) => {
        $crate::aklog::time_log::__push_time_log(format_args!($($arg)*));
    };
}
#[macro_export]
macro_rules! __ak_time_log_dump {
    ($dump:expr) => {
        eprintln!("__ak_time_log_dump");
        $crate::aklog::time_log::__write_time_log_data($dump).expect("ak_time_log_dump failed");
    };
}

#[allow(unused)]
pub use __ak_time_log;
#[allow(unused)]
pub use __ak_time_log_dump;
...
pub fn __push_time_log(args: std::fmt::Arguments) {
    ...
    if data.len() == data.capacity() {
        eprintln!("[AKLOG-WARN] Time log allocation");
    }
    let mut buf = ArrayVec::new();
    if let Err(e) = std::io::Write::write_fmt(&mut buf, args) {
        panic!("ak_time_log failed: {}", e);
    }
...
fn data() -> &'static mut Vec<(i64, ArrayVec<u8, 128>)> {
    unsafe { DATA.as_mut().expect("ak-time-log not initialised") }
}
...
pub(super) fn init() {
    ...
    eprintln!("[AKLOG] ak_time_log is on");
}