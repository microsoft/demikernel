...
impl<const N: usize> UdpQueue<N> {
    pub fn new(rt: Rc<dyn NetworkRuntime<N>>, recv_queue: SharedQueue<N>) -> Self {
        Self {
            local: None,
            rt,
            recv_queue,
...
impl<const N: usize> SharedUdpQueue<N> {
    pub fn new(rt: Rc<dyn NetworkRuntime<N>>, recv_queue: SharedQueue<N>) -> Self {
        Self(SharedObject::new(UdpQueue::new(rt, recv_queue)))
    }
    ...
    pub fn pushto(
        ...
        Ok(self.rt.transmit(Box::new(datagram)))
    }

    pub async fn pop(&mut self, size: Option<usize>, yielder: Yielder) -> Result<(SocketAddrV4, DemiBuffer), Fail> {
        // Currently set to one jumbo frame through the runtime limits, but should eventually be done properly through
        // MTU path discovery.
        let size: usize = size.unwrap_or(limits::POP_SIZE_MAX);
