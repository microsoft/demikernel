...
impl<const N: usize> UdpPeer<N> {
    ...
    pub fn new(
        ...
        let coroutine = Self::background_sender(
            local_ipv4_addr,
            local_link_addr,
            offload_checksum,
            arp.clone(),
        ...
        let task: BackgroundTask = BackgroundTask::new(String::from("Inetstack::UDP::background"), Box::pin(coroutine));
...
impl<const N: usize> SharedUdpPeer<N> {
    ...
    pub fn socket(&mut self) -> Result<QDesc, Fail> {
        ...
        let new_qd: QDesc = qtable.alloc::<SharedUdpQueue<N>>(SharedUdpQueue::new(
            self.rt.clone(),
    ...
    pub fn bind(&mut self, qd: QDesc, mut addr: SocketAddrV4) -> Result<(), Fail> {
        ...
        if self.get_shared_queue_from_addr(&addr).is_some() {
    ...
    pub fn close(&mut self, qd: QDesc) -> Result<(), Fail> {
        #[cfg(feature = "profiler")]
        timer!("udp::close");
        let queue: SharedUdpQueue<N> = self.qtable.borrow_mut().free::<SharedUdpQueue<N>>(&qd)?;
        queue.close()?;
        Ok(())
    }
    ...
    pub fn pushto(&mut self, qd: QDesc, data: DemiBuffer, remote: SocketAddrV4) -> Result<(), Fail> {
        ...
        let queue: SharedUdpQueue<N> = self.get_shared_queue(&qd)?;
    ...
    pub async fn pop(&mut self, qd: QDesc, size: Option<usize>) -> (QDesc, OperationResult) {
        #[cfg(feature = "profiler")]
        timer!("udp::pop");

        // Check that the queue still exists.
        ...
        let result: Result<(SocketAddrV4, DemiBuffer), Fail> = queue.pop(size, yielder).await;
        match result {
    ...
    pub fn receive(&mut self, ipv4_hdr: &Ipv4Header, buf: DemiBuffer) -> Result<(), Fail> {
        ...
        let mut queue: SharedUdpQueue<N> = match self.get_shared_queue_from_addr(&local) {
            Some(queue) => queue,
            None => {
                // Handle wildcard address.
                let local: SocketAddrV4 = SocketAddrV4::new(Ipv4Addr::UNSPECIFIED, hdr.dest_port());
                match self.get_shared_queue_from_addr(&local) {
                    Some(queue) => queue,
    ...
    fn get_shared_queue_from_addr(&self, local: &SocketAddrV4) -> Option<SharedUdpQueue<N>> {
        for (_, boxed_queue) in self.qtable.borrow().get_values() {
    ...
    fn get_shared_queue(&self, qd: &QDesc) -> Result<SharedUdpQueue<N>, Fail> {
        Ok(self.qtable.borrow().get::<SharedUdpQueue<N>>(qd)?.clone())
    }
}
