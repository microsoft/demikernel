...
impl<N: NetworkRuntime> SharedPassiveSocket<N> {
    ...
    async fn poll(mut self) {
        loop {
            ...
            let mut recv_queue: ReceiveQueue = self.recv_queue.clone();
            ...
            futures::select! {
                ...
                result = recv_queue.pop(None).fuse() => {
                    match result {
                        Ok(incoming) =>  {
                                    let remote: SocketAddrV4 = incoming.remote;
                                    ...
                                    let tcp_hdr: TcpHeader = incoming.tcp_hdr;
                                    ...
                                    if !incoming.buf.is_empty() {
                                        ...
                                        warn!("Received SYN with data (len={})", incoming.buf.len());
    ...
    async fn wait_for_ack(
        ...
        if incoming.tcp_hdr.ack_num != local_isn + SeqNumber::from(1) {
