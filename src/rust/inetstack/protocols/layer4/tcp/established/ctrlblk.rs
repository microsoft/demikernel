...
impl<N: NetworkRuntime> SharedControlBlock<N> {
    ...
    pub fn receive(&mut self, incoming: IncomingPacket) {
        self.recv_queue.push(incoming);
    }
    ...
    pub async fn poll(&mut self) -> Result<Never, Fail> {
        ...
        loop {
            let (header, data): (TcpHeader, DemiBuffer) = match self.recv_queue.pop(None).await {
                Ok(incoming) if self.state == State::Established => (incoming.tcp_hdr, incoming.buf),
                Ok(result) => {
    ...
    async fn local_close(&mut self) -> Result<(), Fail> {
        ...
        while self.state != State::TimeWait {
            ...
            let incoming: IncomingPacket = self.recv_queue.pop(None).await?;
            let header: TcpHeader = incoming.tcp_hdr;
    ...
    async fn remote_already_closed(&mut self) -> Result<(), Fail> {
        ...
        loop {
            ...
            // Check ACK.
            match self.process_ack(&incoming.tcp_hdr) {
                Ok(()) => break,
                Err(_) => (),
            }
        }
