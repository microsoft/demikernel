...
impl SharedTcpPeer {
    ...
    pub fn receive(&mut self, src_ipv4_addr: Ipv4Addr, dest_ipv4_addr: Ipv4Addr, tcp_hdr: TcpHeader, buf: DemiBuffer) {
        debug!("TCP received {:?}", tcp_hdr);
        let local: SocketAddrV4 = SocketAddrV4::new(dest_ipv4_addr, tcp_hdr.dst_port);
        ...
        socket.receive(remote, local, tcp_hdr, buf)
    }
}
