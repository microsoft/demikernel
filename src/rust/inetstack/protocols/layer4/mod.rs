...
impl Peer {
    ...
    pub fn bind(&mut self, sd: &mut Socket, socket_addr: SocketAddr) -> Result<(), Fail> {
        ...
        match sd {
            Socket::Tcp(socket) => {
                if *socket_addr_v4.ip() == Ipv4Addr::UNSPECIFIED {
                    // We only support the wildcard address for UDP sockets.
                    // FIXME: https://github.com/demikernel/demikernel/issues/189

                    let cause: String = format!("cannot bind to wildcard address (socket={:?})", socket);
                    error!("bind(): {}", cause);
                ...
                if socket_addr_v4.port() == 0 {
                    let cause: String = format!("cannot bind to port 0 (socket={:?})", socket);
                    error!("bind(): {}", cause);
                    return Err(Fail::new(libc::ENOTSUP, &cause));
                }

                self.tcp.bind(socket, socket_addr_v4)
            },
            Socket::Udp(socket) => {
                ...
                self.udp.bind(socket, socket_addr_v4)
            },
        }?;
