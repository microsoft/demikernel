...
impl Ethernet2Header {
    ...
    pub fn write(&self, writer: &mut Write) -> Result<()> {
        ...
        let ether_type = self.ether_type.clone().into();
        debug!("ether_type -> {:?}", ether_type);
        writer.write_u16::<NetworkEndian>(ether_type)?;
        Ok(())
    }
