...
/// Wait modes for the I/O completion port event processor.
#[derive(Copy, Clone)]
pub enum IoWaitMode {
    /// Wait for events without timing out.
    #[allow(unused)]
    WaitForever,

    /// Wait no longer than a duration.
    #[allow(unused)]
    Timeout(Duration),
...
/// Conditions to return from `IoCompletionPort::process_events`.
#[derive(Copy, Clone)]
enum IoWaitCondition {
    /// Process all queued completions.
    Peek,

    /// Wait until at least one event is woken. If the `Instant` is specified, the condition may additionally be met
    /// when Instant::now() is not earlier than the value.
    WaitWakeOne(Option<Instant>),
}
...
impl IoCompletionPort {
    ...
    /// Process a single overlapped entry. Return true if a coroutine was woken; otherwise, returns false.
    #[allow(unused)]
    fn process_overlapped(&mut self, entry: &OVERLAPPED_ENTRY) -> bool {
    ...
    /// Process completions in the queue. Returns two values: the first indicates whether any yielders were woken; the
    /// second indicates the number of completions dequeued.
    fn process_queued_completions(&mut self, timeout: u32) -> Result<(bool, u32), Fail> {
        const BATCH_SIZE: usize = 4;
        let mut entries: [OVERLAPPED_ENTRY; BATCH_SIZE] = [OVERLAPPED_ENTRY::default(); BATCH_SIZE];
        let mut dequeued: u32 = 0;
        let mut yielder_woken: bool = false;
        match unsafe { GetQueuedCompletionStatusEx(self.iocp, entries.as_mut_slice(), &mut dequeued, timeout, FALSE) } {
            Ok(()) => {
                for i in 0..dequeued {
                    yielder_woken |= self.process_overlapped(&entries[i as usize]);
                }

                Ok((yielder_woken, dequeued))
            },

            Err(err) if err.code() == WIN32_ERROR(WAIT_TIMEOUT.0).into() => Ok((false, 0u32)),

            Err(err)
                if err.code() == WIN32_ERROR(WAIT_ABANDONED.0).into() || err.code() == ERROR_INVALID_HANDLE.into() =>
            {
                Err(Fail::new(libc::EBADF, "completion port closed"))
            },
    ...
    /// Process entries on the completion port. When `wait_mode` indicates the method should wait, the wait will be
    /// satisfied if either the timeout is met or at least one coroutine yielder has been woken. I/O operations which
    /// are abandoned (e.g., a failed cancellation) will not count toward the condition.
    #[allow(unused)]
    pub fn process_events(&mut self, wait_mode: IoWaitMode) -> Result<(), Fail> {
        let (condition, mut timeout): (IoWaitCondition, u32) = IoWaitCondition::from_wait_mode(wait_mode);
        let mut any_yielder_woken: bool = false;

        loop {
            loop {
                let (yielder_woken, dequeued) = self.process_queued_completions(timeout)?;
                any_yielder_woken |= yielder_woken;
                if dequeued == 0 {
                    break;
                }
                timeout = 0;
            ...
            if condition.is_satisfied(any_yielder_woken, now) {
                return Ok(());
            } else {
                timeout = condition.get_timeout(now);
            }
        }
    }
}

impl IoWaitCondition {
    /// Combination operation to convert an IoWaitMode to an IoWaitCondition while returning the initial timeout value.
    /// This reduces the number of system timer checks.
    pub fn from_wait_mode(wait_mode: IoWaitMode) -> (Self, u32) {
        match wait_mode {
            // WaitForever indicates that we wait for the first woken event forever.
            IoWaitMode::WaitForever => (Self::WaitWakeOne(None), INFINITE),

            // Timeout indicates that we wait for the first woken event until the timeout.
            IoWaitMode::Timeout(duration) => (
                Self::WaitWakeOne(Some(Instant::now() + duration)),
                Self::saturate_duration_ms(duration),
            ),

            // PollOnly requires only peeking the completion port.
            IoWaitMode::PollOnly => (Self::Peek, 0u32),
        }
    }

    /// Determine whether the wait condition was satisfied.
    pub fn is_satisfied(self, was_yielder_woken: bool, now: Instant) -> bool {
        match self {
            Self::Peek => true,

            Self::WaitWakeOne(timeout_time) => match timeout_time {
                None => was_yielder_woken,
                Some(timeout_time) => was_yielder_woken || now >= timeout_time,
            },
        }
    }

    /// Get a timeout value for use with GetQueuedCompletionStatus based on the wait condition.
    pub fn get_timeout(self, now: Instant) -> u32 {
        match self {
            Self::Peek => 0,

            Self::WaitWakeOne(timeout_time) => match timeout_time {
                None => INFINITE,
                Some(timeout_time) => Self::saturate_duration_ms(timeout_time - now),
            },
        }
    }

    // Turn a duration into a millisecond quantity saturated to u32.
    fn saturate_duration_ms(duration: Duration) -> u32 {
...
mod tests {
    ...
    fn test_event_processor() -> Result<()> {
        ...
        iocp.process_events(IoWaitMode::PollOnly)?;
    ...
    fn test_overlapped_named_pipe() -> Result<()> {
        ...
        let mut wait_for_state = |scheduler: &mut Scheduler, state| -> Result<(), Fail> {
            while server_state_view.load(Ordering::Relaxed) < state {
                iocp.get_mut().process_events(IoWaitMode::PollOnly)?;
                scheduler.poll();
