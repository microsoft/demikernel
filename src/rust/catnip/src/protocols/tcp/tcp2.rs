...
fn acknowledge(mut sender: Sender, ack_seq_no: SeqNumber) -> Result<(), Fail> {
    ...
    if ack_seq_no > sender.sent_seq_no {
    ...
    if ack_seq_no == sender.sent_seq_no {
        ...
        let deadline = sender.rt.now() + sender.rto.rto();
...
fn update_window(sender: Sender, window_size_hdr: u16) -> Result<(), Fail> {
...
async fn retransmitter(mut sender: Sender) -> Result<!, Fail> {
    loop {
        ...
        futures::select_biased! {
            ...
            _ = rtx_future => {
                ...
                let segment = match sender.unacked_queue.front_mut() {
                    Some(s) => s,
                    None => panic!("Retransmission timer set with empty acknowledge queue"),
                };

                // TODO: Congestion control
                sender.rto.record_failure();
                ...
                let rto = sender.rto.rto();
                sender.retransmit_deadline.set(Some(sender.rt.now() + rto));
            },
        }
    }
}

// TODO: Do we need to piggyback outoging ACKs from the receiver here?
async fn sender(mut sender: Sender) {
    'top: loop {
        ...
        if unsent_seq == sender.sent_seq_no {
            futures::select_biased! {
                _ = win_sz_changed => continue 'top,
        ...
        if window_end <= sender.sent_seq_no as usize {
            futures::select_biased! {
                _ = win_sz_changed => continue 'top,
        ...
        let mut available_window = window_end - sender.sent_seq_no as usize;
        let mut segment = vec![];
        while available_window > 0 {
            let mut buf = match sender.unsent_queue.pop_front() {
                Some(b) => b,
                None => break,
            };
            if buf.len() > available_window {
                let tail = buf.split_off(available_window);
                sender.unsent_queue.push_front(tail);
            }
        ...
        sender.sent_seq_no += segment.len() as i32;
        let unacked_segment = UnackedSegment {
            bytes: segment.clone(),
            initial_tx: Some(sender.rt.now()),
        };
        sender.unacked_queue.push_back(unacked_segment);
