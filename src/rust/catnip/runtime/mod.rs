            ...
            body => {
                ...
                match body {
                    Some(body) if body.len() > 0 => {
                        debug_assert!(body.len() < mbuf.len());
                        mbuf.trim(mbuf.len() - body.len()).expect("Should be able to trim");
                        mbuf.copy_from_slice(&body)
                    },
                    _ if headers_size < MIN_PAYLOAD_SIZE => {
                        let padding_bytes: usize = MIN_PAYLOAD_SIZE - headers_size;
                        mbuf[..padding_bytes].fill(0);
                        mbuf.trim(mbuf.len() - padding_bytes).expect("should be able to trim");
                    },
                    _ => mbuf.trim(mbuf.len()).expect("should be able to trim"),
                }
                mbuf
            },
        };

        // TODO: Remove this expect once we return a value.
        buf.prepend(headers_size).expect("insufficient headroom");
        pkt.write_header(&mut buf[..headers_size]);
        let mut mbuf_ptr: *mut rte_mbuf = expect_some!(buf.into_mbuf(), "mbuf cannot be empty");
