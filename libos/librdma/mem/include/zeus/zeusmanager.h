...
#ifndef ZEUS_ZEUSMANAGER_H
#define ZEUS_ZEUSMANAGER_H
...
namespace Zeus {
    ...
    template <class SourceHeap,
              ...
              class HeapType>
    class ZeusManager : public Hoard::BaseHoardManager<SuperblockType_>,
    ...
    public:
        ...
        {
            Hoard::Check<ZeusManager, sanityCheck> check (this);
            int binIndex;
            size_t realSize;
      ...
      INLINE ibv_mr* rdma_get_mr(void * ptr, rdma_cm_id *rdma_id) {
	SuperblockType * s = SuperHeap::getSuperblock(ptr);
            assert (s->getOwner() == this);
    ...
    private:
        ...
        NO_INLINE void slowPathFree (int binIndex, unsigned int u, unsigned int a) {
            ...
            //	printf ("ZeusManager: this = %x, getting a superblock\n", this);
        ...
        MALLOC_FUNCTION NO_INLINE void * slowPathMalloc (size_t sz) {
            ...
            assert (realSize >= sz);
            for (;;) {
                Hoard::Check<ZeusManager, sanityCheck> check1 (this);
                auto * ptr = getObject (binIndex, realSize);
                if (ptr) {
                    return ptr;
                } else {
                    Hoard::Check<ZeusManager, sanityCheck> check2 (this);
                    // Return null if we can't allocate another superblock.
                    if (!getAnotherSuperblock (realSize)) {
                        //	  fprintf (stderr, "ZeusManager::malloc - no memory.\n");
                        return 0;
                    }
                }
            }
        ...
        class sanityCheck {
        public:
            inline static void precondition (ZeusManager * h) {
                checkInvariant(h);
            }
            inline static void postcondition (ZeusManager * h) {
                checkInvariant(h);
            }
        private:
            inline static void checkInvariant (ZeusManager * h) {
                (void) h;
                assert (h->isValid());
            }
        };
    ...
    private:
        ...
        Hoard::Array<NumBins, Hoard::Statistics> _stats;

        typedef SuperblockType * SuperblockTypePointer;

        typedef Hoard::EmptyClass<SuperblockType, EmptinessClasses> OrganizedByEmptiness;

        typedef Hoard::ManageOneSuperblock<OrganizedByEmptiness> BinManager;

        /// Bins that hold superblocks for each size class.
        Hoard::Array<NumBins, BinManager> _otherBins;
